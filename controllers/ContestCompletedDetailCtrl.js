angular.module('PruForce.controllers')
	.controller('ContestCompletedDetailCtrl', function ($filter,$state, $scope, $rootScope, GetTargetContest,CommonService, $stateParams,$ionicLoading) {

		$scope.viewData = {
			hasHeader: true,
			title: '<h2>' + $filter('translate')('CONTEST.COMPLETED_DETAIL_CONTEST') + '</h2>',
			titleHeader: $filter('translate')('CONTEST.COMPLETED_DETAIL_CONTEST'),
...
//		Get Data Target From Service
		function getTargetContest() {
			GetTargetContest.invoke($scope.DataListFinish.code_contest, $rootScope.agent.code, $rootScope.agent.agentType).then(function (res) {
				$scope.target = res.invocationResult.array;
				for(var i = 0; i<$scope.DataListFinish.contestsCriteria.length; i++){
					var val1 = $scope.target[0][$scope.DataListFinish.contestsCriteria[i].criteriaCategory.name_column_achievement];
					var val2 = $scope.target[0][$scope.DataListFinish.contestsCriteria[i].criteriaCategory.name_column_target];
					if(Number(val1) > Number(val2)){
						$scope.countOl += 1;
					}
					$scope.progressSpinner = ($scope.countOl / $scope.DataListFinish.contestsCriteria.length) * 100;
				}
			}) 
		}
